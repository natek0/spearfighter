...
namespace Spearfighter.Simulation.Tests
{
    ...
    {
        public static SimCore FlatSim(SimConfig cfg = null, uint seed = 1)
            => new SimCore(cfg ?? SimConfig.Default(), seed);

        /// <summary>Step the sim once with the given per-player commands.</summary>
        public static void Step(SimCore sim, params InputCommand[] cmds) => sim.Tick(cmds);

        /// <summary>Hold a single command for n ticks (same command each tick).</summary>
        public static void Hold(SimCore sim, InputCommand cmd, int ticks, int players = 1)
        {
        ...
        public static bool HasEvent(SimCore sim, SimEventType type)
        {
            foreach (var e in sim.Events) if (e.Type == type) return true;
            return false;
        }

        public static int CountEvents(SimCore sim, SimEventType type)
        {
            int n = 0;
